...
function register(j) {
  ...
  j.watch_for(/^\?(?:expressionengine|ee) ([^#@]+)(?:\s*#([1-9]))?(?:\s*@\s*([-\[\]|_\w]+))?$/, function(message) {
    var user = message.match_data[3] || message.user;
    var res = +message.match_data[1] - 1 || 0;

    google.search(message.match_data[1] + ' site:expressionengine.com/user_guide', function(results) {
      if (results.length) {
